...
#define TRUE 1
#define FALSE 0
#define Addr 3 //Address of slave
...
*/
    twst = TWSR & 0xF8;
    while ((twst != TWI_START) && (twst != TWI_REP_START)) ;
/*
** Send device address
*/
...
uint8_t TWIM_ReadNack (void)
	...
	int main (void)
	{
		...
		{
			j=TWIM_Start(Addr,0);
			if(j==FALSE){PORTB=0b11001100; TWIM_Stop();}
